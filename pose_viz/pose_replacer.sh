folders=("Library" "MemorialHall")  # 160 for newest config rendering in cuda 2
...
for ((i=0;i<2;i++)); do
    python ./replace_pose.py -s ${folders[$i]}_opt -n ${folders[$i]}_colmap_high -m $mode -o ../../dataset/ --scale 4.0
done
