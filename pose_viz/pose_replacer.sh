# folders=("Museum" "PeonyGarden") # "MemorialHall" "HaiyanHall" "Museum" "PeonyGarden" "ScienceSquare" "theOldGate" "DayaTemple")  # 160 for newest config rendering in cuda 2
...
for ((i=0;i<1;i++)); do
    python ./replace_pose.py -s ${folders[$i]}_opt -n ${inner_name[$i]} -m $mode -o ../../dataset/
done